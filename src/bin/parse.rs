...
fn main() {
    ...
    for line in input.lock().lines().map(|x| x.unwrap()).filter(|x| !x.starts_with('#')) {
        ...
        let source: u64 = elts[0].parse().ok().expect("malformed source");
        let target: u64 = elts[1].parse().ok().expect("malformed target");
        ...
        edge_writer.write(&unsafe { mem::transmute::<_, [u8; 8]>(target) }).unwrap();
        cur_offset += 1;
    }
